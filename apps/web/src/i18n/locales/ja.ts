...
export default {
  ...
  algorithm: {
    ...
    "time": {
      ...
      "first": "制限時間のある任務では、最初の素材状態の報告から残り時間の見積もりを始めます。手動での調整は不要です。各品目を1個ずつ製作すると仮定し、予備の時間を差し引いて、未完成の品目に残り時間を均等に割り当てます。操作を取り消したり品目を切り替えたりしても、カウントダウンは引き継がれます。",
      "second": "時間に余裕があれば、通常の品質優先の判断を続けます。時間が足りなさそうな場合に限り、CPで耐久を回復して素材状態を待つ回数を減らす方法も比較し、期限内に価値のある品を完成させることを目指します。シミュレーションによる判断のため、最終品質が変わる可能性があり、期限内の完成も保証できません。表示は端末上の見積もりなので、ゲーム内の残り時間も確認してください。"
    },
    "feedback": {
      "title": "実際の結果に合わせて続ける",
