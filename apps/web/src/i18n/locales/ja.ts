...
export default {
  ...
  faq: {
    ...
    algorithmAnswer: "v2.2のソルバーはArtisan's Expert Solverを基に、リソースを活用した手順と最大12アクションの品質最大での完了ルートを探索します。レシピで出現し得るすべての状態で成立することを検証したルートを採用し、各アクションの実際の報告後に再確認します。品質最大が必須のレシピでは、確実な完了ルートがなく、Artisanが状態待ちや失敗する可能性のあるアクションを提案した場合、成功率100%の代替アクションからArtisanに引き継いだ結果もシミュレーションします。2つの状態分布の仮定で十分な根拠が得られた場合に提案を変更します。この比較は推定であり、毎回の成功を保証するものではありません。それ以外はArtisanの提案に従います。",
