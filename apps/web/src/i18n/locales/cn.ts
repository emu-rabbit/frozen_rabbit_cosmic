...
export default {
  ...
  missions: {
    ...
    requiredLevel: '此配方需要等级 {level}，请选择符合等级的装备设置档。',
    ranks: { d: 'D', c: 'C', b: 'B', a: 'A', ex: '高难', 'ex-plus': '高难+', master: '超难' },
  ...
  algorithm: {
    ...
    "time": {
      "title": "时间吃紧时，考虑减少等待",
      "first": "限时任务会从你报告第一颗球色开始，在后台估计剩余时间，不需要手动校准。同一任务先按每个品项制作一件，在扣除预留时间后，平均分给尚未完成的品项；撤回或换品项会沿用原本的倒计时。“再次制作”会保留当前物品与装备，回到第一步，后台倒计时不重置。“重设任务”会重新打开物品与装备选择面板，确认后清除任务进度并重设倒计时，从第一次报告球色重新计时；关闭面板则保留原本进度与倒计时。",
      "second": "时间充裕时，求解器维持原本的品质优先安排。预计来不及时，才额外比较花费 CP 恢复耐久、减少等球的做法，争取在期限内交出有价值的成品。这是根据模拟做出的取舍，可能影响最终品质，也不能保证一定赶得上；后台计时仅供策略判断，实际剩余时间请以游戏内为准。"
    },
    "feedback": {
      "title": "根据你的实际结果继续",
