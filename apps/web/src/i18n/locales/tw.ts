...
export default {
  ...
  algorithm: {
    ...
    "time": {
      "title": "時間吃緊時，考慮減少等待",
      "first": "限時任務會從你回報第一顆球色開始估計剩餘時間，不需要手動校準。同一任務先按每個品項製作一件，在扣除預留時間後，平均分給尚未完成的品項；撤回或換品項會沿用原本的倒數。",
      "second": "時間充裕時，求解器維持原本的品質優先安排。預估來不及時，才額外比較花費 CP 恢復耐久、減少等球的做法，爭取在期限內交出有價值的成品。這是依模擬做出的取捨，可能影響最終品質，也不能保證一定趕得上；畫面倒數是本機估計，仍請留意遊戲內的時間。"
    },
    "feedback": {
      "title": "依你實際做出的結果繼續",
