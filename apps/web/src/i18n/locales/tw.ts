...
export default {
  ...
  missions: {
    ...
    requiredLevel: '此配方需要等級 {level}，請選擇符合等級的裝備設定檔。',
    ranks: { d: 'D', c: 'C', b: 'B', a: 'A', ex: '高難', 'ex-plus': '高難+', master: '超難' },
  ...
  algorithm: {
    ...
    "time": {
      "title": "時間吃緊時，考慮減少等待",
      "first": "限時任務會從你回報第一顆球色開始，在背景估計剩餘時間，不需要手動校準。同一任務先按每個品項製作一件，在扣除預留時間後，平均分給尚未完成的品項；撤回或換品項會沿用原本的倒數。「再次製作」會保留目前物品與裝備，回到第一步，背景倒數不重置。「重設任務」會重新開啟物品與裝備選擇面板，確認後清除任務進度並重設倒數，從第一次回報球色重新起算；關閉面板則保留原本進度與倒數。",
