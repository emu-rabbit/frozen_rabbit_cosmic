...
describe('active craft session input lock', () => {
  it('starts timing at the first reported color, preserves it across item changes and undo, and exports request budgets', async () => {
    ...
    try {
      ...
      expect(craft.state.value?.step).toBe(1)
      expect(craft.activeSession.value?.item.recipeId).toBe(secondItem.recipeId)
      expect(craft.activeSession.value?.equipmentProfile).toEqual(equipmentProfile)
      // Confirming setup for the same mission starts a fresh mission clock.
      startCraftSession({ mission: { ...mission, items: [item, secondItem] }, item, equipmentProfile, crafter })
      await vi.waitFor(() => expect(craft.recommendationLoading.value).toBe(false))
      expect(craft.missionClock.value?.firstReportedConditionAt).toBeNull()
      expect(craft.missionClock.value?.completedRecipeIds).toEqual([])
      expect(recommend.mock.lastCall?.[0].timeBudget).toBeUndefined()
      now += 5_000
      await craft.resolveAction('basicTouch', true, 'normal')
      expect(craft.missionClock.value?.firstReportedConditionAt).toBe(now)
