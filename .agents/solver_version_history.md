...
新增數字版本或改變採用狀態時更新本檔；未採用的描述性實驗由研究 report 保存。`.agents/current_state.md` 只保存目前 baseline／candidate，不重複整條時間線。
...
| v0.30 | `generic-craft-specialist-resource-guard-v0.30.0` | 修正 v0.27 將「耐久 10 但仍有掌握回合」誤判為資源壓力：CP 尚充足時，掌握會覆蓋低耐久訊號，不提前花快速改革；真正低 CP 或沒有掌握覆蓋的低耐久仍可使用。以 state／buff／resource signal 選擇，不讀 recipe／equipment ID。 | 第六批檢測與分析完成；2026-08-27 接受為有足夠改善、保留局部小幅缺陷的架構研究 baseline；implementation checkpoint `59988e2`；[當時 review](../reports/generic-cosmic-overnight/v030-review-20260827/review.md) |
...
| 身份 | 行為差異與用途 | Evidence |
| --- | --- | --- |
| `generic-craft-route-portfolio-v1.13.0` | 球色工作排程與 funded work ownership；完整評測後未採用。 | [v1.13 四表](../reports/generic-cosmic-overnight/generic-native-v113-vs-v112-balanced-e02-e03-e07-e09-e10-2world-64seed-20260902.md) |
| `generic-craft-route-portfolio-v1.14.0` | 在可支付的完工 witness 後開啟排程；相對 Artisan 品質仍不足，未採用。 | [三臂報告](../reports/generic-cosmic-overnight/generic-native-v114-vs-v112-vs-artisan-balanced-e02-e03-e07-e09-e10-2world-64seed-20260903.md) |
| `generic-craft-external-reference-v2.0.0` | Artisan fallback＋三步必成滿品質完工證明；當時採用，現為歷史三步基線。 | [64-seed 四表](../reports/generic-cosmic-overnight/generic-native-full-quality-certificate-vs-artisan-balanced-e02-e03-e07-e09-e10-2world-64seed-20260904.md) |
| `generic-craft-external-reference-v2.1.0` | 證明深度提高為四步，其餘 fallback／逐步重證邊界不變；現為歷史基線。使用者接受相對 v1.12 的品質／完成交換，不是全面支配。 | [採納判讀](../reports/generic-cosmic-overnight/v210-adoption-review-20260907.md) |
| `generic-craft-external-reference-v2.2.0` | 2026-09-06 使用者採用 Artisan continuation 候選並接入 Web。四步資源收尾證明、v1 最多 12 招提案的全部宣告球色驗證與逐步續接；hard-quality、內靜成熟且 Artisan 等球／冒險時，以兩種內部球色假設比較必成首招接回 Artisan 的續作。抽樣比較不是必成證明；未通過仍用 Artisan。保留 v2.1 與實驗身份重播。 | [採用紀錄](../reports/generic-cosmic-overnight/v220-adoption-20260906.md)、[64-seed 判讀](../reports/generic-cosmic-overnight/artisan-continuation-s64-review-20260906.md) |
| `external-reference-exp-*` 系列 | 單招、三至七步證明研究；五步以上未採用。Exact identities 查 Rust source，不能把深度研究等同裝置 gate。 | [單招結果](../reports/generic-cosmic-overnight/external-reference-certified-finish-direct-s8.md)、[三步 readiness](../reports/generic-cosmic-overnight/external-reference-full-quality-certificate-readiness-20260903.md)、[深度研究](../reports/generic-cosmic-overnight/full-quality-certificate-depth-sweet-spot-20260903.md) |
| `exp-normal-route-certificate`／`exp-condition-option-planning` | 已移除的昂貴研究路線；原 run 不續跑。Master extension 的局部收益與負向尾端只作新假說參考。 | [撤回與資訊邊界修正](../reports/generic-cosmic-overnight/condition-information-boundary-and-option-planning-20260831.md) |
| Learned teacher／consensus | 歷史 closed-loop 實驗未取得可採用策略；保留路線混用與估值的失敗教訓。 | [consensus report](../reports/learned-candidate-scorer/teacher-consensus-development-smoke-20260830.md) |

## 閱讀與維護

- 本檔是歷史索引，表中「下一版」「基底」描述當時關係，不指派現在的工作。
- 新方向由 [產品使命](skills/mission/project_mission.md) 擁有；舊實驗結案不永久封鎖 Artisan 強化、蒸餾或自有核心探索。
- v1.12 等舊 evidence 必須區分資訊邊界修正前後的 binary；不能只靠相同 solver 字串沿用結果。
- 數字版號依 [升版規則](skills/professional/development_standards.md)。保存行為差異、用途與可重播 report，不在本檔再抄完整數字、ablation 敘事或下一輪命令。
