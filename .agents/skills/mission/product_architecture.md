...
剩餘時間僅在背景估計，不在製作畫面顯示倒數；演算法說明交代起算與分配方式。求解器只收到 optional 的當件可用時間與操作速度，不讀任務 identity。未傳預算、不限時或尚未開始計時時，只增加有滿品質證明的較短收尾；時間壓力的策略判斷由 Rust 擁有。時間歸零不改寫 mechanics terminal，也不自動停止建議。
